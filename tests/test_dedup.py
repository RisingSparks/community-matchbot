...
async def test_exact_deduplication(db_session, mocker):
    ...
    mock_extractor.extract = mocker.AsyncMock(
        return_value=ExtractedPost(
            confidence=1.0, role="seeker", post_type="mentorship", vibes=[], contribution_types=[]
        )
    )
...
async def test_not_deduplicated_if_different(db_session, mocker):
    ...
    post1 = Post(
        platform=Platform.REDDIT, platform_post_id="r1", raw_text=text1, status=PostStatus.RAW
    )
    db_session.add(post1)
    await db_session.commit()
    await process_post(db_session, post1, mock_extractor)
    ...
    post2 = Post(
        platform=Platform.REDDIT, platform_post_id="r2", raw_text=text2, status=PostStatus.RAW
    )
    db_session.add(post2)
    await db_session.commit()
    await process_post(db_session, post2, mock_extractor)
