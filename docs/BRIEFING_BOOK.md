...
We add value without requiring participants to change their behavior.

## Project Summary: The "Matchbot" Ecosystem
Cross-platform bots that indexe and organize art-, camp-, infra- and mentorship-offering and seeking to facilitate connections without forcing participants into a new tool. 
...
- Generate evidence for broader program decisions. Capture learnings for scalable policy/product integration. 
- Track where matching and support workflows succeed/fail.
- Capture data on what skills are in demand vs. what skills are available.
- Maintain culture and trust while improving access.

### User Type D: Moderators / Community Admins

Jobs:

- Keep channels organized and useful.
- Reduce duplicate/low-context posts.
- Improve thread discoverability without heavy manual curation.
...
- Whether organization alone creates measurable value.
- Whether structured titles improve discoverability and matching quality.
- Whether moderators accept the bot as a time-saving layer.

### Product 2: Infrastructure & Equipment "Bitch n Swap" Bot

**Concept**

Use the same indexing + structure model for infrastructure needs and equipment exchange, especially to help new camps execute safely and affordably.

**In-scope request types**

- Camps seeking tools, shade components, power gear, kitchen infrastructure, transport help, and setup/strike support.
- Camps offering surplus equipment, lending, shared logistics, or experienced guidance.
- Logistics Knowledge Base: Linking matching with "how-to" guides for desert 

**Value**

- Reduces duplicated procurement effort.
- Increases reuse and inter-camp cooperation.
- Lowers operational barriers for new camps and first-time builders.
...
- Event-triggered capture: Process only posts matching target patterns; avoid broad crawling.
3. Bot logs key elements: platform, location preference, vibe, contribution type, what they're offering or seeking.
  - Role type (seeking vs opening)
  - Contribution types / needed roles
  - Availability windows
  - Camp/project descriptors
  - Contact method
3b. Bot maintains active-thread registry with expiration and deduplication.
- Metadata-first storage: Persist structured summaries + canonical links, not full-content archives by default.
4. Optional enrichment: bot joins camp data against What Where When (WWW) Guide to infer camp vibe (wellness, sober, party-focused, etc.).
5. Optional Form Enrichment
- Offer a lightweight form only when better data is needed.
- Form link is an augmentation path, not a hard requirement.
- Preserve source-thread link to keep conversation community-native.
5. Lightweight compatibility triage. Use deterministic parsing first; use LLM extraction for ambiguous cases only.
...
- Number of gear listings indexed (Workstream B).
...
- **Passive-First Architecture**: Build listeners for Reddit/Discord/Telegram before building forms.
- **LLM Parsing Layer**: Use structured JSON output from LLMs to normalize unstructured post data.
